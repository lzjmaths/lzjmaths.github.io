...
There are some notes in this section:
 ...
 \begin{theorem}[Cauchy's theorem for a rectangle]\label{Cauchy's theorem for a rectangle}
   ...
   \end{equation}
      ...
      \end{equation}
       ...
       $ \Rightarrow |{Z}(R^{(n)})| \leq \epsilon d_nL_n=\epsilon \dps\frac{D}{2^n}\cdot\frac{L}{2^n} $ where  $ D $,  $ L $ are the diameter and perimeter of  $ R $.
        ...
        $ \Rightarrow  $  $ |{Z(R)}| \overset{\ref{eq:2}}{ \leq }4^n|{Z}(R^{(n)})| \leq \epsilon DL \Rightarrow Z(R)=0 $ since  $ \epsilon $ is arbitrary.  
\end{proof}    
...
\end{equation}
   ...
   $ \dps\Rightarrow \frac{\partial F}{\partial x}=-i\frac{\partial F}{\partial y} $ $ \Rightarrow  $ $ F  $   is analytic in  $ \Delta $  with derivative  $ f $.
...
\end{lemma}
\begin{proof}
    $ \dps\gamma:\zeta=\zeta(t),\alpha \leq t \leq \beta $.  $ h(t)=\int_\alpha^t\frac{\zeta'(s)}{\zeta(s)-z}\mathrm{d}s $.

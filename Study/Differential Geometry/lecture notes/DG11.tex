...
\begin{theorem}\label{Canonical Form of Commuting Vector Field}
...
\begin{proof}
    ...
    So if we fix  $ t^j,j\neq i $ except  $ t^i $,  $ \psi(t^1,\cdots,t^{i-1},-,t^{i+1},\cdots,t^k,r^{k+1},\cdots,r^{m}) $ is an integral curve for  $ V^i $.  
...
\end{theorem}
\begin{proof}
    This is a local problem. So we may assume  $ M=\mathbb{R}^m,N=\mathbb{R}^n $ by restricting to local charts. And  $ p=0, F(p)=0 $. After changing orders of coordinates, may assume 
    ...
    By inverse function theorem,  $ \exists  $ Nbh  $ U_0\subset \mathbb{R}^m $,  $ \tilde{U}_0\subset \mathbb{R}^m $ of  $ 0 $ \st  $ \varphi:U_0\rightarrow \tilde{U}_0 $ is a diffeomorphism.
    
    \begin{align*}
        \tilde{U}_0&\xrightleftharpoons[\varphi]{\varphi^{-1}}U_0\xrightarrow{F}\mathbb{R}^n\\
        (Q(x,y),y)&\mapsfrom (x,y)\mapsto (Q(x,y),R(x,y))
    \end{align*}
    
    So  $ F\circ \varphi^{-1}:\tilde{U}_i \rightarrow \mathbb{R}^n,(x,y)\mapsto (x,A(x,y))$.
    And 
    \begin{equation}
    ...
    We can find a map  $ \psi:(x,y)\mapsto (x,y-A(x)) $ in a smaller neighborhood of  $ 0 $   by the inverse theorem similarly.

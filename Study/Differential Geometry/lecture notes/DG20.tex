...
\end{proof}
\begin{remark}
    Indeed, completely integrable means an embedding locally. So we can find an embedding integrable manifold locally for immersed integrable manifold.
\end{remark}
\begin{lemma}
...
A  $ k $-dimensional \name{foliation} is a decomposition  $ M=\dps\bigcup_s N_s $ \st (1) Each  $ N_s  $ is an injective immersed  $ k $-dimensional submanifold. (2)  $ \forall p\in M $,  $ \exists  $ local chart  $ (U,x^1,\cdots,x^n) $  \st  $ \forall s\in S $,  $ N_s\cap U=\Rbb^k\times A_s $.  $ A_s $ is a countable subset of  $ \Rbb^{n-k} $.       
\begin{example}

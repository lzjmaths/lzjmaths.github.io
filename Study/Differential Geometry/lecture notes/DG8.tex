...
\end{proof}
\section{Lie group, Lie algebra and Lie bracket}
...
\end{proposition}
\begin{definition}
    Given  $ F\in C^\infty(M,N) $,  $ V\in \mathfrak{T}M $,  $ W\in \mathfrak{T}N $. We say  $ W $ is  \name{$ F $-related} to  $ V $ if  $ \forall p\in M $,  $ F_{p,*}(V_p)=W_{F(p)} $ where $  F_{p,*}:T_pM\rightarrow T_{f(p)}N $      
\end{definition}
\begin{example}
...
\end{proposition}
\begin{corollary}[Naturality of Lie bracket]
    Given any  $ F\in \Diff(M,N) $,  $ V\in \mathfrak{T}M, W\in \mathfrak{T}M  $, we have  $ [F_*V,F_*W]=F_*[V,W] $   
\end{corollary}

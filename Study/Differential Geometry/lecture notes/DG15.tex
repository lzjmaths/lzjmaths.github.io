...
\section{Integration of differential form}
...
\[\alpha=(\alpha_1,\cdots,\alpha_n)^T\sim\beta=(\beta_1,\cdots,\beta_n)^T\Leftrightarrow \det(\alpha\beta^T)\]
...
\[[(e_!,\cdots,e_n)]\leftrightarrow [e_1^*\wedge\cdots\wedge e_n^*]\]
...
\begin{example}
    \,
    \begin{enumerate}
        \item $ U\subset \mathbb{R}^n $ open.  $ U $ has a standard orientation, represented by the form  $ \dd x^1\wedge\cdots\wedge \dd x^n $.

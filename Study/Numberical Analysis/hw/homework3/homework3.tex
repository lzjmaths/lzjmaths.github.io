...
\begin{problem}
    By Theorem 3.3.6, since Chebyshev polynomial is normal w.r.t. the kernal  $ \omega=\frac{1}{\square1-x^2} $ 
    \[\lim_{n\to\infty}\|L_nf-f\|_\omega=0\]
    Noticed that 
    \[\begin{aligned}
        \|L_nf-f\|_\omega&=\int_{-1}^1\omega(x)|L_nf(x)-f(x)|^2\dd x\\
        &
    \end{aligned}\]
\end{problem}
...
\begin{problem}
    ...
    Conversely, if it holds for  $ f=1,\sin x, \cos x $, then for any  $ f\in \Span\{1,\sin x,\cos x\} $,  $ \dps\lim_{n\to\infty}\|L_nf-f\|_\infty=0 $   
\end{problem}
\end{document}
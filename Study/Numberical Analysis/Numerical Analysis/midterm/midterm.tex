...
\[s_nf(x)=\frac{a_0}{2}+\sum_{k=1}^na_k\cos kx+b_k\sin kx\] 
为Fourier级数投影，则
\begin{theorem}
     $ T_\lambda f =f(x+\lambda) $.
     
    \[s_n=\frac{1}{2\pi}\int_{-\pi}^\pi T_{-\lambda}LT_{\lambda}f(x)\dd \lambda\]
    where  $ L $ is the projection on triangular space.   
    \[\|s_n\|=\lambda_n=\frac{1}{\pi}\int_{-\pi}^\pi|D_n(t)|\dd t>\frac{4}{\pi^2}\log n\]
    where  $ D_n(t)=\dps\frac{\sin(n+\frac{1}{2})t}{2\sin\frac{1}{2}t} $ 
\end{theorem}

\begin{theorem}[单调算子定理]
    对于 $ C[a,b] $ 上的单调线性算子序列 $ L_n $，以下条件等价
    \begin{enumerate}
        \item  $ \forall f\in C[a,b] $,  $ \|L_nf-f\|\rightarrow 0 $.
        \item for  $ f=1,x,x^2 $,  $ \|L_nf-f\|\rightarrow 0 $
        \item  $ \|L_n1-1\|_\infty\rightarrow 0    $ and 
        \[(L_n\phi_t)(t)\rightrightarrows 0\text{ uniformly for  $ t $},\,\phi_t(x)=(t-x)^2\]
    \end{enumerate} 
\end{theorem}
\begin{proof}
    Key idea: Find  $ \varphi_t $ such that  $ \varphi_t(t)=0 $, 
    \[|t-x| \leq \alpha \varphi_t(x)\]
    when  $|t-x| $ large.
    
    try treating as  $ f_0=1,f_1=x,f_2=x^2 $ functions.
    
     $ f_0 $ is a functions $ \Rightarrow  $ a constant of  $ x $, like  $ f(t) $ can be treated as functions.   
\end{proof}

\begin{theorem}
    一型边界条件：
    \[\|f-s\|_\infty \leq\frac{5}{384}h^4\|f^{(4)}\|_\infty\]
    \[m_j-f_j'| \geq \frac{1}{24}h^3\|f^{(4)}\|_\infty\]
\end{theorem}

正交多项式内积可以通过 $ (xf_{n},f_{n-2})=(f_{n},xf_{n-2}) $ 转化！

我们有结论
\begin{theorem}
     $ \varphi_n $ 关于 $ \rho $ 的正交多项式序列。 
     \[\varphi_n=(\alpha_n x+\beta_n)\varphi_{n-1}+\gamma_n\varphi_{n-2}\]
\end{theorem}

在考虑正交性的证明的时候，往往要换取别的基证明，或者考虑更一般情况

Legendre多项式和Chebyshev多项式的正交性

\begin{lemma}[Darboux]
    \[\sum_{i=0}^n\bar Q_i(t)\bar Q_i(x)=\lambda_n^{-2}\frac{\bar Q_{n+1}(x)\bar Q_n(t)-\bar Q_n(x)\bar Q_{n+1}(t)}{x-t}\]
    where  $ \lambda_n^{-1} $ is the coefficient of  $ x^n $ term in  $ \bar Q_n $  
\end{lemma}
